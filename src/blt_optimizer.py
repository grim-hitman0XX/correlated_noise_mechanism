...
class BLTOptimizer:
    def __init__(self, n, d, b = 5, k = 10, error_type='rmse', participation='minSep', 
    ...
    def calculate_B_norm_squared(self, alpha_hat, lambda_hat):
        ...
        if self.error_type == 'rmse':
            ...
            for i in range(self.d):
                for j in range(self.d):
                    gamma_hat_ij = self.calculate_Gamma_hat_ij(lambda_hat[i], lambda_hat[j], self.n)
                    B_norm_squared += (1 / self.n) * alpha_hat[i] * alpha_hat[j] * gamma_hat_ij
            ...
            for i in range(self.d):
                for j in range(self.d):
                    gamma_ij = self.calculate_Gamma_ij(lambda_hat[i], lambda_hat[j], self.n)
