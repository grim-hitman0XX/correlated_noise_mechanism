...
class BLTDifferentiableLossOptimizer:
    def __init__(self, n, d, b=5, k=10, participation_pattern = "minSep", error_type='rmse', lambda_penalty=1e-7, 
    ...
    def optimize(self, num_iterations=100, lr=0.001, verbose=False):
        ...
        """
        theta = torch.sort(torch.rand(self.d, device=self.device), descending=True).values
        theta_hat = torch.sort(torch.rand(self.d, device=self.device), descending=True).values
        ...
        for i in range(num_iterations):
            ...
            if loss.item() < best_loss:
                best_loss = loss.item()
                omega = self.calc_output_scale(theta_constrained.detach(), 
                                              theta_hat_constrained.detach(), flag = True)
                omega_hat = self.calc_output_scale(theta_hat_constrained.detach(), 
                                                 theta_constrained.detach(), flag=False)
                best_params = {
